...
For each request, Warden authenticates the identity, evaluates the call against policy at request time, injects the real upstream credential, and proxies it — streaming the response back unchanged. The agent holds no secrets, gets exactly the access its policy permits — no more — and every call is tied to its identity in the audit log.

Warden **need not hold a standing secret at all**. Rather than storing an upstream's key, it can prove *who the caller is* to the upstream and let the upstream mint the credential itself — or fetch a needed secret from an external vault per request and forget it — so there is no secret trove at the gateway to breach. And a request can carry a second identity: the person the agent is acting for. Warden verifies both, so the upstream can authorize the real human, not just the agent — even for systems that have no built-in way to act on someone's behalf.
...
- **Centralized governance** — one control plane for every system an agent touches: one identity, one policy surface, and one audit log across clouds, code hosts, observability, databases, SaaS, and MCP servers. Namespaces isolate teams on shared infrastructure, and Warden rotates any upstream secrets it does hold on a schedule, so they stay fresh without operator coordination.
