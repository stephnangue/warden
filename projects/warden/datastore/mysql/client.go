...
func (c *Conn) Ping() error {
	...
	return vterrors.Errorf(13, "unexpected packet type: %d", data[0])
}
...
func (c *Conn) clientHandshake(params *ConnParams) error {
	...
	case MysqlClearPassword :
		scrambledPassword = []byte(params.Pass)
	default: 
		return sqlerror.NewSQLErrorf(sqlerror.CRSSLConnectionError, sqlerror.SSUnknownSQLState, "Auth plugin %s is not supported", c.authPluginName)
	}
...
func (c *Conn) handleAuthMoreDataPacket(data byte, params *ConnParams) error {
	...
	case CachingSha2FullAuth:
		...
		if c.Capabilities&CapabilityClientSSL > 0 || params.UnixSocket != "" {
			...
			if err != nil {
				return vterrors.Errorf(13, "error encrypting password with public key: %v", err)
			}
...
func parseAuthSwitchRequest(data []byte) (AuthMethodDescription, []byte, error) {
	...
	if !ok {
		return "", nil, vterrors.Errorf(13, "cannot get plugin name from AuthSwitchRequest: %v", data)
	}
...
func (c *Conn) requestPublicKey() (rsaKey *rsa.PublicKey, err error) {
	...
	data[pos] = 0x02
	if err := c.writeEphemeralPacket(); err != nil {
		return nil, vterrors.Errorf(13, "error sending public key request packet: %v", err)
	}
	...
	if err != nil {
		return nil, vterrors.Errorf(13, "failed to parse public key from server: %v", err)
	}
...
func (c *Conn) writeClearTextPassword(params *ConnParams) error {
	...
	// Sanity check.
	if pos != len(data) {
		return vterrors.Errorf(13, "error building ClearTextPassword packet: got %v bytes expected %v", pos, len(data))
	}
...
func (c *Conn) writeScrambledPassword(scrambledPassword []byte) error {
	...
	// Sanity check.
	if pos != len(data) {
		return vterrors.Errorf(13, "error building %v packet: got %v bytes expected %v", c.authPluginName, pos, len(data))
	}
	return c.writeEphemeralPacket()
}
