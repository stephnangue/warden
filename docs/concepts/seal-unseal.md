...
> node — or a whole [HA](high-availability.md) cluster — come up without an
...
sets up the barrier, and issues the first [root token](tokens.md):
...
> [HA](high-availability.md) cluster — restart and open its barrier without a
...
The [dev server](dev-server.md) sidesteps all of this. It uses an in-memory test
...
- [Storage](storage.md) — the backend the barrier encrypts.
- [High Availability](high-availability.md) — why auto-unseal matters for a cluster.
- [Tokens](tokens.md) — the root token that initialization issues.
- [Dev Server](dev-server.md) — auto-init and auto-unseal for local work.
