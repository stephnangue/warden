...
account of who asked for what, which [policy](policies.md) decision was made, and
which [credential](credentials.md) was issued. If a secret is ever misused, the
...
> **The [dev server](dev-server.md) ships with zero audit devices** — it runs
...
[spec](credentials.md) that produced it, its lease, and the token it is bound to
...
**On-behalf-of attribution.** A request can carry a [delegation](delegation.md)
...
- [Policies](policies.md) — the authorization decisions recorded in each entry.
