...
piece of Warden that lives entirely outside the [barrier](seal-unseal.md): by the
...
Warden treats its storage backend as untrusted. The [barrier](seal-unseal.md)
...
— so each [namespace](namespaces.md) and mount reads and writes only under its own
...
loses it on exit — it is what the [dev server](dev-server.md) uses and is never
...
| `ha_table` | Table holding the [HA](high-availability.md) leader-election locks. |
...
See [High Availability](high-availability.md) for how the cluster behaves.

## See Also

- [Seal and Unseal](seal-unseal.md) — the barrier that encrypts everything before
  it reaches storage.
- [High Availability](high-availability.md) — leader election over the storage
  backend.
- [Namespaces](namespaces.md) — the per-namespace storage isolation prefixes.
- [Dev Server](dev-server.md) — the in-memory backend for local work.
