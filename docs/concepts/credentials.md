...
A [role](roles.md) ties this to identity. Its `cred_spec_name` field names the
...
workload actually makes a request to the upstream through a [provider](providers.md),
at which point Warden resolves the role's spec and mints (or reuses) the
credential for it. Because [roles are per-request](roles.md#roles-are-per-request),
...
[namespace](namespaces.md). `-type` and `-rotation-period` are required (use
...
| `TokenID` | The [token](tokens.md) — session or transparent — the credential is bound to and expires with. |
...
Warden [provider](providers.md) mount as if Warden were the upstream. Warden
...
Both sources and specs are namespace-scoped: each [namespace](namespaces.md) has
...
- [Namespaces](namespaces.md) — the isolation boundary for sources and specs.
