...
For local development you can register a trust domain from a static bundle generated by a local SPIRE server (or any SPIFFE-conformant CA), then exercise both SVID types against it. The method's unit-test suite mints X.509-SVIDs and JWT-SVIDs from locally-generated keypairs — no running SPIRE deployment needed — and verifies trust-domain isolation, the audience requirement, the JWT-SVID-wins precedence, TTL capping by SVID expiry, and federation refresh.
