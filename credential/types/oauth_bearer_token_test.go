...
func TestOAuthBearerTokenCredType_ValidateConfig(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:       "token_exchange source - missing subject source",
			config:     map[string]string{"audience": "https://api.example.com"},
			sourceType: credential.SourceTypeTokenExchange,
			wantErr:    true,
			errMsg:     "is required for a token_exchange source",
		},
		{
			name:       "token_exchange source - subject source none",
			config:     map[string]string{credential.ConfigSubjectTokenSource: credential.SourceNone},
			sourceType: credential.SourceTypeTokenExchange,
			wantErr:    true,
			errMsg:     "is required for a token_exchange source",
		},
