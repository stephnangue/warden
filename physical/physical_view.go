...
// View represents a prefixed view of a physical storage
type View struct {
	backend Storage
	prefix  string
}
...
var _ Storage = (*View)(nil)

// NewView takes an underlying physical storage and returns
// a view of it that can only operate with the given prefix.
func NewView(backend Storage, prefix string) *View {
	return &View{
		backend: backend,
		prefix:  prefix,
	}
