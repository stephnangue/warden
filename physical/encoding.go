...
var (
	_ Storage              = &storageEncoding{}
	_ TransactionalStorage = &transactionalStorageEncoding{}
	_ Transaction          = &storageEncodingTransaction{}
)

// NewStorageEncoding returns a wrapped physical storage and verifies the key
// encoding
func NewStorageEncoding(b Storage) Storage {
	se := &storageEncoding{
		Storage: b,
	}
...
func (e *storageEncoding) Purge(ctx context.Context) {
	if purgeable, ok := e.Storage.(ToggleablePurgemonster); ok {
		purgeable.Purge(ctx)
	}
}
...
func (e *transactionalStorageEncoding) BeginReadOnlyTx(ctx context.Context) (Transaction, error) {
	txn, err := e.storageEncoding.Storage.(TransactionalStorage).BeginReadOnlyTx(ctx)
	if err != nil {
		return nil, err
	}
...
func (e *transactionalStorageEncoding) BeginTx(ctx context.Context) (Transaction, error) {
	txn, err := e.storageEncoding.Storage.(TransactionalStorage).BeginTx(ctx)
	if err != nil {
		return nil, err
	}
...
func (e *storageEncodingTransaction) Commit(ctx context.Context) error {
	return e.storageEncoding.Storage.(Transaction).Commit(ctx)
}

func (e *storageEncodingTransaction) Rollback(ctx context.Context) error {
	return e.storageEncoding.Storage.(Transaction).Rollback(ctx)
}