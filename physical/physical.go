...
// CacheInvalidationBackend is an extension to the standard storage
// backend to support cache invalidation. Warden can serve read requests
...
// FencingHAStorage is an HAStorage which provides the additional guarantee that
...
type FencingHAStorage interface {
	HAStorage
...
const (
	...
	// fencing through to a FencingHAStorage. Note that this is not an ideal use
	...
	// than needing to specialize every physical.Storage to understand this
	// option.
	unfencedWriteContextKey unfencedWriteContextKeyType = "warden-disable-fencing"
)
...
type Factory func(config map[string]string, log logger.Logger) (Storage, error)

// Entry is used to represent data stored by the storage
type Entry struct {
	Key      string
	Value    []byte
...
func (e *Entry) String() string {
	return fmt.Sprintf("Key: %s. Value: %s. ValueHash: %s", e.Key, hex.EncodeToString(e.Value), hex.EncodeToString(e.ValueHash))
}
