...
- **RFC 8693 token exchange reshaped around the user; the `header` token source and `auth_token` value are retired.** The subject/actor model is reworked so delegation forwards the *end user's own validated credential*, and every forwarded token is now trusted at the source — there is no caller-supplied, per-request-validated token path.
  - The `subject_token_source`/`actor_token_source` value **`auth_token` is renamed to `agent_identity`** (unchanged meaning: the agent's verified inbound JWT). It is a shipped v0.18.0 value — **migrate** each spec by renaming `-config=subject_token_source=auth_token` / `-config=actor_token_source=auth_token` to `=agent_identity`.
  - The **`header` token source is removed.** Agent-acting-for-a-user delegation now uses the new **`subject_token_source=user_identity`**, which forwards the secondary *user* principal's own auth-method-validated credential as the RFC 8693 `subject_token` — replacing the old caller-supplied `X-Warden-Subject-Token` header that the driver validated per request. `user_identity` is pinned to a `token_exchange` source and fails closed when no user principal was presented (the user is authenticated by secondary transparent authentication — see the per-user credential-chaining feature). The spec keys **`subject_token_header`/`actor_token_header`** and the source trust keys **`subject_issuer`/`subject_audience`/`subject_jwks_url`/`subject_oidc_discovery_url`** are removed along with the driver's per-request `validateUntrustedToken`.
  - The delegation rule changed: an actor is only meaningful in the delegation shape, so **`actor_token_source ≠ none` now requires `subject_token_source=user_identity`.** Previously `actor=warden_identity` required `subject=header`; now `subject=warden_identity + actor=agent_identity` (and any actor paired with a non-`user_identity` subject) is **rejected**.
  - The internal token-**origin** concept is gone: the audit credential-metadata keys **`subject_verified`/`actor_verified` are no longer emitted** (every forwarded token is verified at the source); the `subject`/`actor` identity keys are unchanged.
  - **Migration.** A create/update carrying a retired value (`subject_token_source`/`actor_token_source` = `header` or `auth_token`, or the removed `subject_*`/`*_token_header` keys) is rejected by the schema; a **persisted** spec with one fails closed **at mint** (with a message naming the retired value) rather than minting without exchange/delegation. Rename `auth_token`→`agent_identity`, and re-express a header-subject delegation as `subject_token_source=user_identity` with a provider/namespace `user_auth_path` configured. Removed source/spec keys left in config are otherwise ignored. The `X-Warden-Subject-Token`/`X-Warden-Actor-Token` request headers are no longer read (they stay on the upstream strip-lists, harmless). (#468, #469, #476)

- **GitHub credential specs select the credential with `mint_method`, not `auth_method`.** The `github_token` driver now reads a required spec-level `mint_method` key (`app` | `pat`) to choose what to mint, matching every other credential driver's convention (`mint_method` = which credential to mint; `auth_method` = how a *source* authenticates to its backend). The old `auth_method` key is no longer accepted on a `github_token` spec: setting it is rejected at create/update with a message pointing to `mint_method`. **Migrate** each GitHub spec by renaming `-config=auth_method=<app|pat>` to `-config=mint_method=<app|pat>`. (GitLab is unaffected — it legitimately uses both keys: `auth_method`=`pat`/`oauth2` for how it authenticates, `mint_method` for what it mints.)

- **`X-Warden-On-Behalf-Of` retired; audit/policy actors are verified-only.** The self-reported on-behalf-of request header is removed — the audit/policy actor chain now has a single source: the cryptographically-verified RFC 8693 `act` claim from the caller's JWT/SPIFFE token. Because every actor is now verified, the `verified` field is dropped from the actor shape everywhere: the audit `actors[]` objects lose the `"verified"` JSON key (`{subject, verified}` → `{subject}`), and CEL `token.actors` elements expose only `{subject}`. **A policy `condition` that references `a.verified` (e.g. `token.actors.all(a, a.verified)`) now fails closed:** `token` is a map type, so the expression still compiles but errors at evaluation ("no such key") and the request is **denied** — rewrite such conditions to `size(token.actors) > 0` (verification is now implicit). Note the rewrite also tightens the empty-chain case: a bare `token.actors.all(a, a.verified)` was vacuously **true** (allow) for a token with no actors, whereas `size(token.actors) > 0` is **false** (deny) — so a token that carries no `act` chain is now denied by such a condition; if allow-when-empty was intended, drop the condition (the old expression allowed every request). This actor-chain change is unrelated to the Entra `token_param.requested_token_use=on_behalf_of` parameter, which is unchanged. (The former `X-Warden-Subject-Token` / `X-Warden-Actor-Token` `header` token source is retired separately — see the RFC 8693 exchange reshaping above.)

### New Features

- **Warden is now an OIDC issuer — the foundation for Workload Identity Federation.** Warden can mint short-lived signed JWT *identity assertions* describing the authenticated agent (`sub = wid:{namespaceID}:{mountAccessor}:{principalID}`, plus `warden_sub`, `warden_role`, `warden_namespace`, `warden_auth_mount`) and publish the matching JWKS and OIDC discovery document, so any upstream that trusts an OIDC issuer — AWS STS, Entra, GCP STS, an OpenBao/Vault JWT auth mount, an OAuth token endpoint — can verify an agent's identity directly, regardless of how that agent authenticated to Warden. Configured with `warden oidc-issuer configure` (`issuer_url` + timings); the issuer fails closed until an active signing key is installed, and key rotation retires the prior key into the JWKS so in-flight assertions keep verifying. The auth-mount accessor is persisted on the token so a federated subject is globally unique, and the credential cache is keyed on a stable identity rather than the per-request assertion bytes. (#436, #451, #458, #466)

- **The OIDC-issuer signing key can live in an external KMS.** A `signer { }` server-config stanza binds the issuer's assertion-signing key to an external KMS so the **private key never leaves the KMS** — Warden keeps only a handle plus the cached public key and asks the KMS to sign each assertion. `type = "transit"` (OpenBao/Vault) ships today; the `oidcsign.Backend` interface is built to add more backends (cloud KMS, PKCS#11 HSM). A storage v4 key format serializes a key as either a local PKCS#8 key or a remote handle; in-process keys remain the default when no `signer` stanza is set. `warden oidc-issuer read` shows the signing-key location and a `key_rotation` status block. (#452, #453, #454, #455, #460)

- **OIDC-issuer publishers expose the public JWKS/discovery to internet-facing upstreams, with self-rotating write credentials.** A *publisher* pushes the issuer's discovery + JWKS documents to a public location (bucket/CDN) that becomes the `issuer_url` upstreams fetch from — so Warden itself need not be internet-exposed. Publisher types: `local_file`, `http_put`, `s3`, `gcs`, `azure_blob`, configured with `warden oidc-issuer set-publisher -type=<...> -config=key=value` (`-type=none` removes it). On signing-key rotation the updated JWKS is automatically re-published, gated by a propagation floor so Warden only begins signing with a key once it is live in the published document. Warden also self-rotates the publisher's own write credential on a cadence (a fresh IAM access key for `s3`, a rotated service-account/OAuth secret for `gcs`, a rotated service-principal client secret via Microsoft Graph for `azure_blob`). (#438–#446, #478)

- **Keyless credential sources — `auth_method=oidc_federation`.** A credential source can hold **no stored cloud secret**: on each request Warden mints its identity assertion for the caller and exchanges it at the cloud for a short-lived credential. `mint_method` names *what* to produce; the source's `auth_method` decides *how* Warden authenticates (stored key vs. keyless federation), and `subject_token_source` selects whether the subject is a Warden-issued assertion (`warden_identity`) or the agent's own inbound JWT (`agent_identity`). Ships for **AWS** (`sts_assume_role` via `AssumeRoleWithWebIdentity`, and keyless Secrets Manager reads), **Azure** (`bearer_token` via an Entra `client_assertion`; `key_vault_secret` is not federated and fails closed), **GCP** (Workload Identity Federation at `sts.googleapis.com`, `access_token` / `impersonated_access_token`), and **OpenBao/Vault** (per-request JWT-auth login — keyless sources need no `rotation_period`, keyless `vault_token` specs need no `token_role`). The assertion `aud` can be derived once from the source's federation trust and auto-applied per driver, and `assertion_algorithm` selects RS256 (default) or ES256. (#447–#449, #457, #458, #459, #461, #463, #464, #465, #467)

- **Chain a spec's secret from another cred spec (`secret_spec`).** A consuming source/spec that needs a standing secret does not store it: it names another cred spec via `secret_spec`, and at request time Warden mints that referenced spec **as the same caller**, extracts the material (`secret_field`), and hands it to the consuming driver — so a secret-backed provider becomes keyless at Warden. Nothing is persisted in Warden's barrier for a chained spec; the material only transits memory at mint time, and every read is a per-request, authorized, audited event attributed to the real caller. Bounded to a single hop (`MaxSecretChainDepth = 1`); a referenced spec cannot be deleted while in use (`ErrSpecInUse`). Explicitly defeats the "key-trove" class where a compromised broker leaks a pile of standing secrets. (#473, #479)

- **`kv2_read` mint method + `key_value` credential type.** A generic OpenBao/Vault KV v2 read whose payload is preserved verbatim (structure-agnostic, no forced primary field), so a referenced `secret_spec` can carry material under its natural key names and persists no secret in its own config. (#472)

- **The `token_exchange` client secret can be sourced via chaining.** A `token_exchange` source may set `secret_spec` (+ optional `secret_field`) to fetch its `client_secret` — or the `private_key` for `client_auth=private_key_jwt` — from another cred spec at mint time instead of storing it inline; `client_id` stays in config and the inline secret is omitted. This makes the exchange itself keyless at Warden, and applies to `client_secret_basic`/`_post`, `private_key_jwt`, and both legs of an ID-JAG exchange. (#481)

- **Opt-in source-scoped cache for chained secrets (`secret_cache_ttl`).** A chained consumer can cache the fetched secret material for a bounded TTL, keyed on the source rather than the consuming spec, to avoid re-fetching a shared upstream secret on every request. Off unless set. (#480)

- **Per-user `credential_name` templating for the `oauth2` mint method.** The `oauth2` mint method's `credential_name` accepts `{{user.<claim>}}` templating, so a per-user OAuth token is selected by a verified user claim (complementing the per-user `secret_path` templating on `kv2_read`). (#477)

- **Per-user credential chaining via secondary transparent authentication.** A gateway request can carry a second, per-request *user* credential (default header `X-Warden-User-Token`) alongside the agent's own; Warden authenticates it through the existing auth methods into a first-class *user* principal — identity-only, so it never authorizes the request (the agent token alone does). It is configured per provider/namespace with `user_auth_path` (+ optional `user_token_header`/`user_auth_role`), and a bearer-format auth mount is required; a missing user credential is not an error (downstream flows that need it fail closed on their own), while a present-but-invalid, cross-namespace, or agent-equal credential fails closed. A `subject_token_source=warden_identity` spec may set **`assertion_user_claims`** to disclose the user under a nested **`warden_user`** claim — the user's own `sub` (raw principal) plus allowlisted metadata, with **`warden_sub`** carrying the agent's raw principal — so a downstream templated/trust policy scopes per user; the same projection templates a per-user **`secret_path`** (`{{user.<claim>}}`, sanitized against path traversal) for a chained `kv2_read`, and the `oauth2` driver can mint from a chained (per-user) refresh token. The minted credential is cached per user and bounded by both principals' token TTLs, so revoking or expiring either the agent or the user stops new mints. The raw user credential is scrubbed from the request after capture and never audited (only the user identity is stamped for attribution). (#475)

- **`warden_identity` assertions can carry allowlisted token metadata.** A credential spec with `subject_token_source=warden_identity` may set `assertion_metadata_claims` (comma-separated) to project selected login-derived token-metadata keys into the minted assertion, so a verifier that evaluates arbitrary bound claims (an external OpenBao/Vault `jwt` auth mount, or Warden's own JWT auth method) can gate on attributes such as `team` or `env` without Warden minting a distinct upstream role per value. (AWS/Azure STS federation trust policies bind only on `sub`/`aud`, so the nested `warden_metadata` claim is informational there — the same verifier-side caveat as `warden_resource` below.) The projection is opt-in and least-disclosure: absent/empty projects nothing (unchanged behavior), only the named keys are included — never the whole metadata map — and they ride under a single nested `warden_metadata` claim so they cannot clobber a registered or `warden_*` claim. The projected set is size-capped and fails the request closed if exceeded, and it is folded into the credential-cache key so two tokens that share subject and audience but differ in projected metadata never share a cached upstream credential.
- **`warden_identity` assertions name the target resource.** A minted assertion now carries a top-level `warden_resource` claim naming the single downstream resource the credential spec targets, so a verifier that evaluates arbitrary bound claims can pin one assertion to one resource (`bound_claims = { "warden_resource": "<res>" }`) — least privilege at the token level. The value is auto-derived from the source/spec config: AWS `secrets_manager` → `aws-secretsmanager:<secret_id>`, AWS `sts_assume_role` → `aws-iam:<role_arn>`, Azure `bearer_token` → `azure:<resource_uri>`, and `token_exchange` with a single RFC 8707 resource → `oauth-resource:<uri>`; a spec with no single, statically-known resource carries no claim. Derivation is pure config reads — no live driver is built and nothing touches the network on the request path. Set `assertion_resource` to override the derived value verbatim, or `assertion_resource=none` to suppress the claim (opt-out); the key is valid only with `subject_token_source=warden_identity`. The claim value is opaque (its provider prefix is human-readable, never parsed — a resource id may itself contain `:`). **Enforcement is verifier-side and out of scope here:** AWS/Azure federation trust policies bind only on `sub`/`aud`, so there the claim is informational; it is enforced only by a verifier that checks arbitrary bound claims (an external Vault/OpenBao `jwt` auth mount configured to trust Warden's issuer, or Warden's own JWT auth method), which must be presented the assertion by out-of-band wiring. **On upgrade**, any `warden_identity` spec with a derivable resource emits an additional claim and its exchange cache key changes (a one-time re-mint); set `assertion_resource=none` to keep the prior assertion bytes. (#450)
