...
- **CLI success/empty messages are structured in non-table output modes.** Scripts running in a non-TTY context (which now autodetects to `json`) previously saw plain-text strings on stdout. Affected commands include the generic `read`/`write`/`delete`, every typed mutating subcommand (`auth enable/disable`, `audit enable/disable`, `provider enable/disable`, `cred source create/update/delete`, `cred spec create/update/delete`, `namespace create/update/delete`, `policy write/delete`), and the empty-list path of every `list` subcommand. The plain-text `Success! ...` / `Successfully deleted ...` / `No providers enabled` strings are now JSON envelopes: `{"path": "...", "written": true}` for write, `{"path": "...", "deleted": true}` for delete, `{"path": "...", "type": "...", "enabled": true}` for enable, etc.; `[]` for empty lists. Create commands that previously printed multi-line confirmations include the same fields in the envelope (`name`, `id`, `custom_metadata`, `rotation_period`, `config`, ...). Scripts that grep for the human strings need to update; agents that parse JSON were already broken by the human strings and now work correctly. Pass `-o table` to keep the human messages.

### Documentation

- **Agent skill catalog under top-level `skills/`.** Runtime-agnostic guides for autonomous agents calling services through Warden. The catalog separates *agent runbooks* (terse, action-focused) from *operator tutorials* (the existing `provider/<name>/README.md` files): READMEs are humans walking through full setup, skills are what the agent reads at request time. Foundation files (`warden-shared`, `discovery`, `troubleshooting`) cover global flags, the auth → roles → list-providers → match-task loop, and classified-error handling. Six worked-example provider skills cover one provider per SDK framework category — `aws` (sigv4), `vault`/`github`/`openai` (httpproxy), `scaleway` (dualgateway), `rds` (dbaccess) — and ship the contribution template (`AGENTS.md` documents the shape) so each new provider includes a matching `skills/providers/<name>/SKILL.md` alongside its README. Repo root gets `AGENTS.md` as the agent entry point pointing into the tree.

### New Features

- **`--json` payload on every mutating typed command.** Adds a `--json` / `-j` flag accepting a literal JSON object (`--json '{"type":"aws",...}'`), a file reference (`--json @aws.json`), or `-` for stdin (`cat aws.json | warden ... --json -`) on every typed mutating command: `cred source create/update`, `cred spec create/update`, `auth enable`, `audit enable`, `namespace create/update`, `provider enable`. Mutually exclusive with the per-typed-flag surface (`--type`, `--config`, etc.); combining the two errors with exit `2` (`usage`). For commands where the mount path can be derived from the type (`auth enable`, `audit enable`, `provider enable`), the path is taken from the positional argument when given, otherwise from the payload's `type` field. Composes cleanly with `--dry-run`: agents can iterate `build payload → --dry-run → fix typos → real call` against any mutating endpoint without writing the typed-flag flattening glue. The previously-required `--type`, `--source`, `--rotation-period`, `--metadata` flags become required-unless-`--json`. Server-side fix bundled: `handleCredentialSourceUpdate` now preserves `RotationPeriod` from the existing source on update — without this, every `hvault` source update failed validation (`rotation_period is required`), and non-`hvault` updates silently stripped their rotation configuration.
- **Global `--dry-run` / `-D` flag — local schema validation, no server round-trip.** The flag never sends a request: it fetches the operation's schema from `/v1/sys/schema?path=<path>` (cached per process), validates the agent's payload structurally — required fields, unknown field names with Levenshtein "did you mean" hints, JSON type matches, enum/allowed-value checks — and either exits `0` with a `{"path", "validated": true, "dry_run": true}` envelope or exits `3` (`invalid_input`) with the structured error envelope listing every problem at once. Honors `WARDEN_DRY_RUN` (any value other than empty / `0` / `false` / `no` / `off`). Wired into every mutating CLI surface: `warden write` / `warden delete`, `warden auth enable/disable`, `warden audit enable/disable`, `warden provider enable/disable`, `warden cred source create/update/delete`, `warden cred spec create/update/delete`, `warden namespace create/update/delete`, `warden policy write/delete`. The validator catches the failure mode that matters most for agents: hallucinated parameters. Authz/RBAC and conflict checks are NOT verified because the request never reaches the server — those failures surface only when the agent runs the real command, and the cost of that is "you get an error response" rather than data loss. Server-side enhancement bundled: `projectOASToPath` now matches concrete paths against templated keys (`sys/cred/sources/my-aws` finds `sys/cred/sources/{name}`), so `warden schema PATH` works without agents knowing placeholder names.
- **`warden role list` CLI command** — agent-facing role introspection over the server's `/v1/sys/introspect/roles` aggregator. Lists the `{name, description, auth_path}` of every role the caller's identity (JWT bearer or TLS client cert) can assume across the namespace's auth mounts. Lives under a `warden role` parent group for symmetry with `warden provider list`. Composes with `--output` and `--fields`; per-mount failures surface on stderr without changing the exit code so a partial-failure mount can't mask the rest of the namespace's roles. `--auth-path` filters client-side. The CLI client (`cmd/helpers/client.go`) now also promotes a JWT-shaped `WARDEN_TOKEN` (one starting with `eyJ`) to an `Authorization: Bearer` header so identity-introspection endpoints — which specifically don't read `X-Warden-Token` — work without a separate env var. Server-side fix bundled: `sys/introspect/roles` is now unauthenticated by `PathsSpecial`, matching the per-mount `auth/{jwt,cert}/introspect/roles` paths. The handler self-authenticates via the presented JWT/cert (returns 401 if neither). Without this, an agent presenting only its identity vehicle (no Warden token) got 403 from the policy check before the handler ran — defeating the whole point of introspection.
- **`warden schema` CLI command** — agent-facing introspection that hits the server's `/v1/sys/schema` endpoint and projects the OpenAPI doc into a friendly shape. Three modes:
  - `warden schema PATH` — describe a single path. Returns `{path, methods, description, parameters, response_schema, auth_required}` with body fields merged from `requestBody.content."application/json".schema.properties` and `sensitive` flag preserved from `x-vault-displayAttrs`. The `description` is sourced from the path-item's `description` (`HelpSynopsis`), falling back to the first operation's `summary`/`description` if the path-item has none.
  - `warden schema --list` — enumerate every path in the caller's namespace. Output is NDJSON-friendly: `{path, methods}` per line.
  - `warden schema PATH --raw` — emit the raw OpenAPI fragment for tools that already speak OpenAPI (Stainless, openapi-typescript, oapi-codegen).
  Composes with the global `--output`/`--fields` flags from PR 1 and the structured-error/exit-code framework from PR 2: unknown path → exit `6` (`not_found`); usage errors → exit `3`.
  Server-side fixes bundled in this PR: provider and auth-method mounts (previously hidden behind wrapper types) now appear in the schema; body parameters described via `$ref` are resolved into the friendly view; `path` not found returns 404 instead of 500; gateway streaming paths are intentionally excluded since their OAS shape would mislead codegen.
- **`warden path-help` honors the global `--output` framework.** Previously the command did a raw `fmt.Print` of the help text — agents in a non-TTY context still got plain stdout. Now it returns `{"help": "..."}` in `json`/`ndjson`/`text` modes and prints the prose verbatim in `table` mode. Path argument is validated via `helpers.ValidatePath`. Missing-help responses now exit `6` (`not_found`) instead of silently exiting `0`. Pairs with `warden schema` to give agents one structured surface for params and one for prose.
...
- **Input hardening at the CLI boundary.** Three validators in `cmd/helpers/path.go` reject malformed inputs before any HTTP call, with errors classified as exit code `3` (`invalid_input`):
  ...
  - `ValidateIdentifier` — `--type` on `cred source create` and `cred spec create`. Rejects empty, slashes, and the same hazards.
