...
- **`warden roles` CLI command** — agent-facing role introspection over the server's `/v1/sys/introspect/roles` aggregator. Lists the `{name, description, auth_path}` of every role the caller's identity (JWT bearer or TLS client cert) can assume across the namespace's auth mounts. Composes with `--output` and `--fields`; per-mount failures surface on stderr without changing the exit code so a partial-failure mount can't mask the rest of the namespace's roles. `--auth-path` filters client-side. The CLI client (`cmd/helpers/client.go`) now also promotes a JWT-shaped `WARDEN_TOKEN` (one starting with `eyJ`) to an `Authorization: Bearer` header so identity-introspection endpoints — which specifically don't read `X-Warden-Token` — work without a separate env var. Server-side fix bundled: `sys/introspect/roles` is now unauthenticated by `PathsSpecial`, matching the per-mount `auth/{jwt,cert}/introspect/roles` paths. The handler self-authenticates via the presented JWT/cert (returns 401 if neither). Without this, an agent presenting only its identity vehicle (no Warden token) got 403 from the policy check before the handler ran — defeating the whole point of introspection.
