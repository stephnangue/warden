...
- **Global `--dry-run` / `-D` flag (client-side plumbing).** Stamps `X-Warden-Dry-Run: true` on every outbound request so a future server-side enforcement pass can short-circuit mutating handlers. Honors `WARDEN_DRY_RUN` (any value other than `0`/`false`/`no`/`off`/empty enables it). Until server enforcement ships, the CLI emits a single stderr warning per invocation — *the request will still be processed normally* — so users can wire dry-run into their workflows today and the behavior automatically tightens once enforcement lands. The warning is on stderr in every output mode so agents in `-o json` see it without polluting stdout.
