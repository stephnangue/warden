...
- **Default audit device uses an absolute path and fails soft on unwritable filesystems.** The bootstrap audit device that `loadAudits` creates on first unseal moves from the relative `warden-audit.log` (resolved against the process working directory) to the absolute `/var/log/warden/audit.log`, overridable via the new `WARDEN_DEFAULT_AUDIT_PATH` env var. When the path can't be opened (read-only rootfs, missing parent dir, permission denied), the three default-creation failure paths now log a warn with a remediation hint and return nil instead of aborting unseal — closing the worst symptom of the relative-path bug where unseal would fail *after* barrier writes had happened, leaving the cluster initialized-but-corrupted without ever returning the root token. The stored-table branch (operator-enabled devices reloaded from storage) keeps its strict failure semantics. Helm chart drops the `workingDir=/tmp` + explicit-command workaround and mounts a new `/var/log/warden` emptyDir so the absolute default works under `readOnlyRootFilesystem=true`; chart version bump `0.2.0` → `0.2.1`. A follow-up will move audit-device declaration into the HCL config to remove the chicken-and-egg where the operator can't bootstrap an audit device via `sys/audit/{path}` if zero devices are registered. (this PR)
