...
type Handler interface {
	...
	// the method below are for management only

	// set the principal id obtained after the authentication
	SetPrincipal(principalID string)
	// set the role name obtained after the authentication
	SetRole(roleName string)
	// stop the handler
	// this method will close the backend connection
	Stop()
}
