...
func Test(t *testing.T) {
	...
	for _, svr := range servers {
		...
		err := inMemProvider.AddUser(*testUser, *testPassword)
...
func (s *serverTestSuite) onConn(conn net.Conn) {
	...
	co, err := s.server.NewConn(conn, &MockAuthResolver{}, s.credProvider, &testHandler{s})
	require.NoError(s.T(), err)
	// set SSL if defined
	for {
...
func (h *testHandler) SetPrincipal(principalID string) {
}

func (h *testHandler) SetRole(roleName string) {
}
