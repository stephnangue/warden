...
// ProviderConfig holds parsed configuration for the GitHub provider
...
func parseConfig(conf map[string]any) ProviderConfig {
	...
	if addr, ok := conf["github_url"].(string); ok && addr != "" {
		config.GitHubURL = addr
	}

	if ver, ok := conf["api_version"].(string); ok && ver != "" {
...
func ValidateConfig(conf map[string]any) error {
	// github_url is optional (defaults to api.github.com), but if provided must be valid
	if addr, ok := conf["github_url"].(string); ok && addr != "" {
...
// validateGitHubAddress validates that the github_url is a well-formed HTTPS URL
func validateGitHubAddress(addr string) error {
	if addr == "" {
