...
// ProviderConfig holds parsed configuration for the Slack provider
...
func parseConfig(conf map[string]any) ProviderConfig {
	...
	if addr, ok := conf["slack_url"].(string); ok && addr != "" {
		config.SlackURL = addr
	}
...
func ValidateConfig(conf map[string]any) error {
	// slack_url is optional (defaults to slack.com/api), but if provided must be valid
	if addr, ok := conf["slack_url"].(string); ok && addr != "" {
...
// validateSlackAddress validates that the slack_url is a well-formed HTTPS URL
func validateSlackAddress(addr string) error {
	if addr == "" {
	...
	if err != nil {
		return fmt.Errorf("invalid slack_url: %w", err)
	}

	if parsed.Scheme != "https" {
		return fmt.Errorf("slack_url must use https:// scheme, got: %s", parsed.Scheme)
	}

	if parsed.Host == "" {
		return fmt.Errorf("slack_url must include a host")
	}
