...
// pathConfig returns the config path definition for the Slack provider
func (b *slackBackend) pathConfig() *framework.Path {
	return &framework.Path{
		Pattern: "config",
		Fields: map[string]*framework.FieldSchema{
			"slack_url": {
				Type:        framework.TypeString,
				Description: "The Slack API base URL (default: https://slack.com/api)",
				Default:     DefaultSlackURL,
			},
			"max_body_size": {
		...
		Operations: map[logical.Operation]framework.OperationHandler{
			logical.ReadOperation: &framework.PathOperation{
				...
				Summary:  "Read Slack provider configuration",
			},
			logical.UpdateOperation: &framework.PathOperation{
				Callback: b.handleConfigWrite,
				Summary:  "Configure Slack provider settings",
			},
		},
		HelpSynopsis:    "Configure Slack provider",
		HelpDescription: "This endpoint configures the Slack provider settings including API URL, body size limits, and timeouts.",
	}
}

// handleConfigRead handles reading the Slack provider configuration
func (b *slackBackend) handleConfigRead(_ context.Context, _ *logical.Request, _ *framework.FieldData) (*logical.Response, error) {
...
// handleConfigWrite handles writing the Slack provider configuration
func (b *slackBackend) handleConfigWrite(ctx context.Context, _ *logical.Request, d *framework.FieldData) (*logical.Response, error) {
	if val, ok := d.GetOk("slack_url"); ok {
		addr := val.(string)
		if addr != "" {
			addr = strings.TrimRight(addr, "/")
			if err := validateSlackAddress(addr); err != nil {
				return &logical.Response{
