...
func init() {
	sharedTransport = newSlackTransport()
...
// newSlackTransport creates an HTTP transport optimized for Slack API workloads
func newSlackTransport() *http.Transport {
	transport := &http.Transport{
		...
		// Timeout settings — Slack API is fast, unlike AI inference
	...
	if err := http2.ConfigureTransport(transport); err != nil {
		log.Printf("Failed to configure HTTP/2 for Slack transport: %v", err)
	}
