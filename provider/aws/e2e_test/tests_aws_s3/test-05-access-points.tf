...
# Note: Using the underlying bucket for object creation since the AWS provider
# may not properly resolve access point ARNs through a proxy. The access point
# is still tested for access control purposes.
resource "aws_s3_object" "ap_test_create" {
  bucket = aws_s3_bucket.access_point.id
  key    = "test-files/access-point-create.txt"
  content = jsonencode({
    message     = "Created for Access Point test"
    timestamp   = timestamp()
    test_number = "30"
    operation   = "CREATE"
  })
  ...
  depends_on = [aws_s3_access_point.main]
}

# Test object - UPDATE via Access Point (same key, different content)
# Note: Using the underlying bucket since AWS SDK doesn't properly support
# Access Point ARNs with custom endpoints (AWS_ENDPOINT_URL)
resource "aws_s3_object" "ap_test_update" {
  bucket = aws_s3_bucket.access_point.id
  key    = "test-files/access-point-update.txt"
  content = jsonencode({
    message     = "Updated for Access Point test"
    timestamp   = timestamp()
    test_number = "30"
    operation   = "UPDATE"
    version     = "2"
...
# KNOWN LIMITATION: MRAP data plane operations (PutObject, GetObject, etc.) cannot be
# proxied through Warden because:
# 1. MRAP uses SigV4A signing (AWS4-ECDSA-P256-SHA256), not standard SigV4
# 2. AWS SDK resolves MRAP ARN to virtual-hosted URL and bypasses AWS_ENDPOINT_URL
# See provider/aws/README.md for details.
#
...
# VPC Access Point data operations require running from within the VPC.
# This test will fail with AccessDenied when running from outside the VPC.
#
...
output "test_objects_created" {
  value = {
    ...
    # mrap_east commented out - MRAP data operations bypass Warden (see provider/aws/README.md)
    # vpc_ap commented out - VPC Access Points require running from within the VPC
  }
