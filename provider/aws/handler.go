...
func (p *AWSProvider) handleConfig(w http.ResponseWriter, r *http.Request) {
	switch r.Method {
	case http.MethodGet:
...
func (p *AWSProvider) handleGetConfig(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.NewEncoder(w).Encode(config); err != nil {
		p.logger.Error("failed to encode config response",
			logger.Err(err),
			logger.String("request_id", middleware.GetReqID(r.Context())),
		)
...
func (p *AWSProvider) handleUpdateConfig(w http.ResponseWriter, r *http.Request) {
	var incomingConfig map[string]interface{}
	if err := json.NewDecoder(r.Body).Decode(&incomingConfig); err != nil {
		p.logger.Warn("failed to decode config request",
			logger.Err(err),
			logger.String("request_id", middleware.GetReqID(r.Context())),
		)
	...
	maps.Copy(mergedConfig, incomingConfig)
