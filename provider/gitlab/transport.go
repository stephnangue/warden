...
func init() {
	sharedTransport = newGitLabTransport()
...
// newGitLabTransport creates an HTTP transport optimized for GitLab API workloads
func newGitLabTransport() *http.Transport {
	...
	if err := http2.ConfigureTransport(transport); err != nil {
		log.Printf("Failed to configure HTTP/2 for GitLab transport: %v", err)
	}
