...
- **Standard API** — Injects `Authorization: Bearer` header with the IAM token. Covers VPC, Kubernetes, Resource Controller, and all other IBM Cloud services.
...
Configure the provider with `auto_auth_path`:
...
path "ibmcloud/role/+/gateway/v2/resource_instances" {
  capabilities = ["read"]
}

path "ibmcloud/role/+/gateway/v2/vpc/*" {
  capabilities = ["read"]
}
EOF
```
...
Requests use role-based paths. Warden performs implicit JWT authentication and injects the IBM Cloud IAM token automatically.

The URL pattern is: `/v1/ibmcloud/role/{role}/gateway/{api-path}`

Export IBM_ENDPOINT as environment variable:
```bash
export IBM_ENDPOINT="${WARDEN_ADDR}/v1/ibmcloud/role/ibmcloud-user/gateway"
```

### List Resource Instances

```bash
curl -s "${IBM_ENDPOINT}/v2/resource_instances" \
...
```bash
curl -s "${IBM_ENDPOINT}/v1/vpcs?version=2024-06-01&generation=2" \
  -H "Authorization: Bearer ${JWT_TOKEN}" \
  -H "Content-Type: application/json"
```
...
```bash
curl -s "${IBM_ENDPOINT}/v2/vpc/getClusters" \
  -H "Authorization: Bearer ${JWT_TOKEN}" \
  -H "Content-Type: application/json"
```
...
```bash
curl -s "${IBM_ENDPOINT}/v2/resource_groups" \
  -H "Authorization: Bearer ${JWT_TOKEN}" \
  -H "Content-Type: application/json"
```
...
curl --cert client.pem --key client-key.pem \
    ...
    -s "https://warden.internal/v1/ibmcloud/role/ibmcloud-user/gateway/v2/resource_instances" \
    -H "Content-Type: application/json"
```
...
|-------|------|---------|-------------|
| `ibmcloud_url` | string | `https://cloud.ibm.com` | IBM Cloud API base URL |
