	...
	b.StreamingBackend = &framework.StreamingBackend{
		StreamingPaths: []*framework.StreamingPath{
			...
			// Role-based gateway paths for implicit auth
			{
				...
				HelpSynopsis:    "Vault Gateway proxy",
				HelpDescription: "Proxies requests to HashiCorp Vault with role embedded in URL path",
			},
			{
				Pattern:         "role/[^/]+/gateway/.*",
				Handler:         b.handleTransparentGatewayStreaming,
...
Warden performs implicit authentication on every request and obtains a Vault
token from the credential manager — generated by a Vault source driver using
the configured auth method and role — and injects it as the X-Vault-Token
header in the proxied request. This allows Warden to broker Vault access
without distributing long-lived Vault tokens to clients.
