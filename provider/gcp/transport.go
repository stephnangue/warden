...
// newTransport creates an HTTP transport optimized for GCP workloads
func newTransport() *http.Transport {
	transport := &http.Transport{
		// Connection pool settings
		// Higher limits for parallel GCP API calls
		MaxIdleConns:        200,
		...
		// HTTP/2 optimization — GCP APIs consistently support HTTP/2
	...
	if err := http2.ConfigureTransport(transport); err != nil {
		...
		log.Printf("GCP provider: Failed to configure HTTP/2: %v", err)
	}
