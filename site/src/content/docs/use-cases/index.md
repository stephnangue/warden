...
credentials to production. These six pages are the problems that tension creates,
...
They build on one another: [access brokering](/use-cases/access-brokering/) keeps the
...
[per-user access](/use-cases/per-user-access/) carries the real person behind the agent
through to the upstream; [centralized governance](/use-cases/centralized-governance/) is
where all of that is operated from one place; and
[audit & attribution](/use-cases/audit-attribution/) records what happened.
