...
[Authentication](/concepts/authentication/); this section sits on top of
...
| A1 | [Sidecar Token Injection](/agent-identity/sidecar-injected-token/) (Robin) | an `Authorization: Bearer` token the sidecar adds | `kubernetes` / `spiffe` |
| A2 | [Sidecar mTLS Tunnel](/agent-identity/sidecar-tunneled-cert/) (ghostunnel) | nothing — identity rides the mTLS channel the sidecar originates | `cert` / `spiffe` |
| **B — Agent-Presented Identity** | | *The identity-aware agent attaches its own credential.* | |
| B1 | [Self-Minted SVID](/agent-identity/self-minted-identity/) | an SVID the agent fetched from the Workload API | `spiffe` |
| B2 | [Platform Token Relay](/agent-identity/platform-issued-token/) | a token the platform issued (projected SA token, CI OIDC) | `kubernetes` / `jwt` |
...
- [Authentication](/concepts/authentication/) — the credential forms, auth
  methods, and transparent auth these approaches build on.
- [Tokens](/concepts/tokens/) — what authentication produces.
- [Agent Flow](/agent-flow/) — the end-to-end path once identity is
  established.
