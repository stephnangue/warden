...
even when the model is confused or deliberately misled. See [MCP](/concepts/mcp/) for how
...
[audit log](/concepts/audit/) — with that role. Because a step's role maps one-to-one to
...
| Field | Meaning |
|-------|---------|
| `token_policies` | The [policies](/concepts/policies/) attached to tokens issued for this role — the projection that decides what the role can do (and, for MCP mounts, which tools its `tools/list` returns). |
...
> See [Token Types](/concepts/tokens/#token-types).
...
[Expiration and renewal](/concepts/tokens/#expiration-and-renewal). When a token expires,
...
- [Tokens](/concepts/tokens/) — what a role issues.
- [Credentials](/concepts/credentials/) — what `cred_spec_name` points at.
- [Audit](/concepts/audit/) — where each action is recorded, stamped with its role.
- [Namespaces](/concepts/namespaces/) — the scope roles and their tokens live in.
