...
runs several nodes against one shared [storage](/concepts/storage/) backend so that the
...
[PostgreSQL](/concepts/storage/) backend, nodes contend for a single advisory lock (the
...
actually do work, and it is why **[auto-unseal](/concepts/seal-unseal/) matters for HA**:
...
- [Storage](/concepts/storage/) — the shared backend and the `ha_enabled` lock table.
