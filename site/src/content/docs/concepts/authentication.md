...
> the plain-HTTP [dev server](/concepts/dev-server/) listener.
...
initialized. On a [dev server](/concepts/dev-server/) it is printed in the startup
...
|--------|------|-----------|
| [**Certificate**](/auth-methods/cert/) | `cert` | an X.509 client certificate against configured rules |
| [**JWT / OIDC**](/auth-methods/jwt/) | `jwt` | a signed JWT against an OIDC discovery URL, JWKS URL, or static public keys |
| [**Kubernetes**](/auth-methods/kubernetes/) | `kubernetes` | a Kubernetes ServiceAccount token via the cluster's TokenReview API |
| [**SPIFFE**](/auth-methods/spiffe/) | `spiffe` | a SPIFFE identity — accepts **either** an X.509-SVID or a JWT-SVID |
...
Policy evaluation itself is covered in [Policies](/concepts/policies/); [Namespaces](/concepts/namespaces/)
covers the isolation boundary, and the [Architecture](/architecture/) overview
