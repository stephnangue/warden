...
authenticates in the first place, see [Authentication](/concepts/authentication/).
...
**Transparent tokens** back [transparent authentication](/concepts/authentication/#transparent-authentication).
...
The four role types correspond one-to-one with the [auth methods](/concepts/authentication/#auth-methods).
...
[role](/concepts/roles/) the caller authenticated against.
...
| `Policies` | The [policies](/concepts/policies/) granted to the caller. |
| `NamespaceID` / `NamespacePath` | The [namespace](/concepts/namespaces/) the token was issued in. |
...
| `Metadata` | Verified, login-derived identity attributes (e.g. `env`, `team`) mapped from the caller's JWT claims, certificate fields, or service-account attributes — never caller-supplied. Matched by [policy conditions](/concepts/policies/#fine-grained-access) as `agent.metadata.<key>` (or `user.metadata.<key>` for the user principal). |

Unlike the other fields, **`Metadata` is how per-identity attributes reach
policy**. Each auth method maps trusted parts of the verified identity onto
metadata keys — the [JWT](/auth-methods/jwt/) and SPIFFE JWT-SVID methods via
`metadata_claims`, and the [certificate](/auth-methods/cert/),
[Kubernetes](/auth-methods/kubernetes/), and [SPIFFE](/auth-methods/spiffe/)
X.509 methods via `metadata_mappings`. Because the values come from the verified
credential and never from the request, a [CEL `condition`](/concepts/cel-conditions/)
can trust `agent.metadata.<key>` to gate access, and the same attributes are
recorded in the [audit log](/concepts/audit/) in clear by default — so every
...
- **Created at initialization** — printed once in the [dev server](/concepts/dev-server/)
...
- [Authentication](/concepts/authentication/) — how a caller proves identity and obtains
  a token.
- [Roles](/concepts/roles/) — how an auth method maps a credential to policies and token
  settings.
- [Policies](/concepts/policies/) — what a token is authorized to do.
- [Namespaces](/concepts/namespaces/) — the isolation boundary every token is bound to.
