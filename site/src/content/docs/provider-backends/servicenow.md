...
<p align="center"><img alt="An agent presents the user's ID token and its own identity to Warden, which builds an assertion carrying user and agent claims, has an external KMS sign it, reads the ServiceNow API token from an external vault at a path templated by the agent's team and environment, and injects it to the ServiceNow API" src="/images/warden-prov-servicenow-vault-apikey.png" width="860"></p>
...
5. Warden authenticates to the **external vault** and reads
   `secret/servicenow/{{agent.team}}/{{agent.env}}`.
...
<p align="center"><img alt="Warden reads a static ServiceNow API token from its encrypted storage and injects it to the ServiceNow API for every caller" src="/images/warden-prov-servicenow-inline-apikey.png" width="860"></p>
...
| Mode | What ServiceNow sees | Where the token lives |
|---|---|---|
...
ServiceNow exposes no workload-identity federation, so a long-lived token exists in both rows;
...
warden cred spec create servicenow-ops -json '{
  ...
  "config": {
    ...
    "kv2_mount": "secret",
    "secret_path": "servicenow/{{agent.team}}/{{agent.env}}"
  }
}'
```
...
```bash
warden cred source create servicenow-src -json '{
  "type": "apikey",
  "config": {
    "api_url": "https://mycompany.service-now.com",
    "display_name": "ServiceNow"
  }
}'

printf '{"source":"servicenow-src","min_ttl":3600,"max_ttl":86400,"config":{"api_key":"%s"}}' \
  "$(cat /path/to/servicenow-token)" | warden cred spec create servicenow-ops -json -
```
...
ServiceNow supports OAuth2 client-credentials, but **this mount cannot use it today**. The
...
you need OAuth2 against ServiceNow, the generic [`rest` provider](/provider-backends/rest/)
