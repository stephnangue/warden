...
A workload whose SA is `system:serviceaccount:prod:inventory-agent` yields metadata `ns="prod"`, `sa="inventory-agent"`. A policy can then gate a path with `condition = "agent.metadata.ns == 'prod'"`. Unknown attribute selectors are rejected at role write time.
