...
// InitializedLocally checks if Warden is already initialized from the
...
// Initialize is used to initialize Warden with the given
// configurations.
func (c *Core) Initialize(ctx context.Context, initParams *InitParams) (*InitResult, error) {
	...
	if c.seal.RecoveryKeySupported() {
		...
		if err != nil {
			...
			c.logger.Error("failed to read recovery configuration", logger.Err(err))
			return results, nil
		}
...
func (c *Core) initializeInternal(ctx context.Context, initParams *InitParams) (*InitResult, error) {
	...
	if c.seal.RecoveryKeySupported() {
		...
		if err := recoveryConfig.ValidateRecovery(); err != nil {
			c.logger.Error("invalid recovery configuration",
				logger.Err(err),
			)
	...
	if c.ha != nil {
		...
		if err != nil {
			c.logger.Error("got error acquiring lock",
				logger.Err(err),
			)
		...
		defer func() {
			if err := lock.Unlock(); err != nil {
				c.logger.Error("failed to unlock initialization lock",
					logger.Err(err),
				)
			}
		}()
	...
	err = c.seal.Init(ctx)
	if err != nil {
		c.logger.Error("failed to initialize seal",
			logger.Err(err),
		)
	...
	if err := c.barrier.Initialize(ctx, barrierKey, sealKey, c.secureRandomReader); err != nil {
		c.logger.Error("failed to initialize barrier",
			logger.Err(err),
		)
	...
	if err := c.barrier.Unseal(ctx, barrierKey); err != nil {
		c.logger.Error("failed to unseal barrier",
			logger.Err(err),
		)
	...
	defer func() {
		...
		if err := c.barrier.Seal(); err != nil {
			c.logger.Error("failed to seal barrier",
				logger.Err(err),
			)
		}
	}()
	...
	if err != nil {
		c.logger.Error("failed to save barrier configuration",
			logger.Err(err),
		)
	...
	if err := c.postUnseal(activeCtx, ctxCancel, standardUnsealStrategy{}); err != nil {
		c.logger.Error("post-unseal setup failed during init",
			logger.Err(err),
		)
		return nil, err
	}
	...
	if c.seal.RecoveryKeySupported() {
		...
		if err != nil {
			c.logger.Error("failed to save recovery configuration",
				logger.Err(err),
			)
		...
		if recoveryConfig.SecretShares > 0 {
			...
			if err != nil {
				c.logger.Error("failed to generate recovery shares",
					logger.Err(err),
				)
				return nil, err
			}
	...
	if err != nil {
		c.logger.Error("root token generation failed",
			logger.Err(err),
		)
		return nil, err
	}
	results.RootToken = rootToken
	...
	if initParams.RootTokenPGPKey != "" {
		...
		if err != nil {
			c.logger.Error("root token encryption failed",
				logger.Err(err),
			)
			return nil, err
		}
	...
	if err := c.preSeal(); err != nil {
		c.logger.Error("pre-seal teardown failed",
			logger.Err(err),
		)
		return nil, err
	}
...
// return value of "nil" implies the Warden instance is unsealed.
//
...
func (c *Core) UnsealWithStoredKeys(ctx context.Context) error {
	...
	sealed := c.Sealed()
	if !sealed {
		c.logger.Warn("attempted unseal with stored keys, but warden is already unsealed")
		return nil
	}
	...
	if c.Sealed() {
		// This most likely means that the user configured Warden to only store a
		// subset of the required threshold of keys. We still consider this a
		// "success", since trying again would yield the same result.
		c.logger.Warn("warden still sealed after using stored unseal key")
	} else {
