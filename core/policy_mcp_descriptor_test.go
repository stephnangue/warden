...
// mcpDescriptorFromHeaders builds a single-call descriptor with
// MatchArgs populated only for param-names the policy mentions and
// that have header values. Pins the contract between adapter and
// matcher.
func TestMCPDescriptorFromHeaders_Shape(t *testing.T) {
	sets := []*CBPMCPRules{{
		AllowedMethods: []string{"tools/call"},
		AllowedTools:   []string{"read_file"},
		AllowedParams: map[string][]string{
	...
	req := newMCPRequest(t, "mcp/gateway/", map[string]string{
		"Mcp-Method":     "TOOLS/CALL", // case-insensitive, lowercased by adapter
		"Mcp-Name":       "Read_File",
		"Mcp-Param-Path": "docs/readme.md",
		// encoding header absent — adapter must not invent it
	})
	desc := mcpDescriptorFromHeaders(req, sets)
	if desc == nil || len(desc.Calls) != 1 {
		t.Fatalf("descriptor = %+v, want 1 call", desc)
	}
	c := desc.Calls[0]
	if c.Method != "tools/call" {
		t.Errorf("Method = %q, want lowercased tools/call", c.Method)
	}
	if c.Name != "read_file" {
		t.Errorf("Name = %q, want lowercased read_file", c.Name)
	}
	if c.MatchArgs == nil {
		t.Fatalf("MatchArgs = nil, want populated for path")
	}
	if pv, ok := c.MatchArgs["path"]; !ok || pv.Str != "docs/readme.md" || pv.Kind != logical.ParamString {
		t.Errorf("path arg = %+v, want String/docs/readme.md", pv)
	}
	if _, ok := c.MatchArgs["encoding"]; ok {
		t.Error("encoding present in MatchArgs but no header was sent")
	}
}

// mcpDescriptorFromHeaders skips MatchArgs population entirely for
// non-tools/call methods — there's no param gate to honour.
func TestMCPDescriptorFromHeaders_NonToolsCallSkipsParams(t *testing.T) {
	sets := []*CBPMCPRules{{
		AllowedMethods: []string{"tools/list"},
		AllowedParams: map[string][]string{
			"path": {"docs/*"},
	...
	desc := mcpDescriptorFromHeaders(req, sets)
	if desc.Calls[0].MatchArgs != nil {
		t.Errorf("MatchArgs = %v, want nil for non-tools/call", desc.Calls[0].MatchArgs)
	}
}
