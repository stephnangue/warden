...
func (d *autoSeal) SetCore(core *Core) {
	d.core = core
	if d.logger == nil {
		d.logger = d.core.logger.WithSystem("autoseal")
	}
}
...
func (d *autoSeal) BarrierConfig(ctx context.Context) (*SealConfig, error) {
	...
	if err != nil {
		d.logger.Error("failed to read seal configuration", log.String("seal_type", sealType), log.Err(err))
		return nil, fmt.Errorf("failed to read %q seal configuration: %w", sealType, err)
	}

	// If the seal configuration is missing, we are not initialized
	if entry == nil {
		d.logger.Info("seal configuration missing, not initialized", log.String("seal_type", sealType))
		return nil, nil
	}
	...
	if err != nil {
		d.logger.Error("failed to decode seal configuration", log.String("seal_type", sealType), log.Err(err))
		return nil, fmt.Errorf("failed to decode %q seal configuration: %w", sealType, err)
	}

	// Check for a valid seal configuration
	if err := conf.Validate(); err != nil {
		d.logger.Error("invalid seal configuration", log.String("seal_type", sealType), log.Err(err))
...
func (d *autoSeal) SetBarrierConfig(ctx context.Context, conf *SealConfig) error {
	...
	if err := d.core.physical.Put(ctx, pe); err != nil {
		d.logger.Error("failed to write barrier seal configuration", log.Err(err))
...
func (d *autoSeal) SetRecoveryConfig(ctx context.Context, conf *SealConfig) error {
	...
	if err := d.core.physical.Put(ctx, pe); err != nil {
		d.logger.Error("failed to write recovery seal configuration", log.Err(err))
...
func (d *autoSeal) SetRecoveryKey(ctx context.Context, key []byte) error {
	...
	if err := d.core.physical.Put(ctx, be); err != nil {
		d.logger.Error("failed to write recovery key", log.Err(err))
...
func (d *autoSeal) getRecoveryKeyInternal(ctx context.Context) ([]byte, error) {
	...
	if err != nil {
		d.logger.Error("failed to read recovery key", log.Err(err))
...
func (d *autoSeal) StartHealthCheck() {
	...
	go func() {
		...
		fail := func(msg string, args ...interface{}) {
			fields := make([]log.TypedField, 0, len(args)/2)
					...
					if err != nil {
						...
						func() {
							...
							if !bytes.Equal([]byte(testVal), plaintext) {
								...
								if !lastTestOk {
									d.logger.Info("seal backend is now healthy again", log.String("downtime", t.Sub(lastSeenOk).String()))
