...
func buildAuditAuth(auth *logical.Auth, te *logical.TokenEntry) *audit.Auth {
	...
	// The verified RFC 8693 "act" chain flows in on the token entry (persisted
	// from the JWT "act" claim at login). auth.Actors is never populated at
	// request time, so te.Actors is the sole source.
	if te != nil && len(te.Actors) > 0 {
